...
from evaluation.attacks.attack_utils import count_masks, apply_extracted_fills
from evaluation.models import Model, ReferenceModel
...
class NeighborhoodAttack(Attack):
    ...
    def _attack(self, document, probs, **kwargs):
        ...
        if loss is None:
            loss = self.target_model.get_ll(document, probs=probs)
...
class T5Model(MaskFillingModel):
    ...
    def generate_neighbors(self, texts, **kwargs) -> List[str]:
        ...
        outputs = []
        for i in range(0, len(texts_use), chunk_size):
            outputs.extend(
